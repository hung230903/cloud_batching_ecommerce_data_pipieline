...
def _normalizer_processing(data, schema_type):
    """
    ...
    if pa.types.is_struct(schema_type):
        ...
        for i in range(schema_type.num_fields):
            ...
            filtered[field.name] = _normalizer_processing(val, field.type)
        return filtered
    ...
    if pa.types.is_list(schema_type):
        ...
        return [_normalizer_processing(item, item_schema) for item in data]
...
def normalize_product_info_data(df):
    ...
    for name in schema.names:
        ...
        # apply() func: apply a lambda func for each value in the DF
        df[name] = df[name].apply(lambda x: _normalizer_processing(x, field_type))
    return df
